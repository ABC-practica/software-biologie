...
public class ObjectViewerController {
    ...
    private void handleUpload() {
        ...
        fileChooser.getExtensionFilters().add(
                new FileChooser.ExtensionFilter("3D Mesh Files (*.obj, *.3mf)", "*.obj", "*.3mf")
        );
    ...
    private void startRenderer(ScanMesh mesh, File file) {
        ...
        RenderStrategy newRenderer = RenderStrategyFactory.createRenderer(mesh);
        renderer = newRenderer;

        newRenderer.embedIn(viewportContainer);

        int vertexCount = mesh.getVertices() != null ? mesh.getVertices().length / 3 : 0;
        int faceCount = mesh.getIndices() != null ? mesh.getIndices().length / 3 : 0;

        updateUIState(
                file.getName(),
                String.format("Active: %s | Vertices: %,d | Faces: %,d | Controls: Drag to rotate, Right drag to pan, Scroll to zoom",
                        file.getName(), vertexCount, faceCount)
        );
    }
    ...
    private void updateUIState(String fileName, String statusMessage) {
        if (activeFileLabel != null) {
            activeFileLabel.setText(fileName != null ? fileName : "No model loaded");
        }
